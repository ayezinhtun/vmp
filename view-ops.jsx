// Operations views — TasksView (drag-drop kanban), AlertsView, ActivityView, NetworkView

const TasksView = ({ openVM, openCust, openModal }) => {
  const { state, moveTask, deleteTask, addTask } = useStore();
  ...
  return (
    <div className="content">
      ...
      {openTask && <ProvisioningRequestDetail taskId={openTask} onClose={() => setOpenTask(null)}/>}
    </div>
  );
};
...
const ActivityView = () => {
  ...
  const map = { 'VM': 'vm', 'Finance': 'finance', 'Task': 'task', 'Alert': 'alert', 'Customer': 'customer' };
  const kindIcon = { vm: 'server', finance: 'invoice', task: 'tasks', alert: 'bell', customer: 'users' };
  const kindColor = { vm: 'var(--accent)', finance: 'var(--ok)', task: 'var(--info)', alert: 'var(--warn)', customer: 'oklch(0.55 0.17 285)' };
                  ...
                  {['vm', 'finance', 'task', 'alert', 'customer'].map(k => (
                    <div key={k} className="flex center between" style={{ padding: '6px 0' }}>
                      <span className="flex center gap-2 text-sm">
                        <span style={{ width: 26, height: 26, borderRadius: 7, background: `${kindColor[k]}1a`, color: kindColor[k], display: 'grid', placeItems: 'center' }}><Icon name={kindIcon[k]} size={13}/></span>
...
const AlertsView = () => {
  ...
  const [search, setSearch] = React.useState('');
  const [selected, setSelected] = React.useState(null);
  const [prefs, setPrefs] = React.useState({ d30: true, d7: true, d1: true, d0: true, grace: false, email: true, inapp: true, teams: false });
  const togglePref = (k) => setPrefs(p => ({ ...p, [k]: !p[k] }));
  const sevColor = { urgent: 'var(--bad)', warn: 'var(--warn)', info: 'var(--info)' };
  const sevLabel = { urgent: 'Urgent', warn: 'Warning', info: 'Info' };

  const typeIcon = { expiry: 'clock', kyc: 'shield', finance: 'invoice', task: 'tasks', system: 'settings', vm: 'server' };
  ...
  const open = (a) => { markAlertRead(a.id); setSelected(a); };
...
const NetworkView = ({ openVM, openModal }) => {
...
Object.assign(window, { TasksView, ActivityView, AlertsView, NetworkView });
