...
const AccountSettingsView = ({ role, setView }) => {
  const { state, updateMember, toast } = useStore();

  // Map current role to a team member identity
  const meByRole = {
    'Admin': state.team.find(t => t.role === 'Admin'),
    'Sales': state.team.find(t => t.role === 'Sales'),
    'Engineer': state.team.find(t => t.role === 'Engineer'),
    'Finance': state.team.find(t => t.role === 'Finance'),
  };
  const me = meByRole[role] || state.team[0];
  ...
  React.useEffect(() => { setProfile(p => ({ ...p, name: me.name, email: me.email })); }, [me.id]);
            ...
            <div className="flex col gap-3">
              ...
              {[
                ...
                <div key={key} className="flex center between">
                  ...
                  <span className={`toggle ${notif[key] ? 'on' : ''}`} onClick={() => setNotif({ ...notif, [key]: !notif[key] })}/>
                </div>
              ))}
              <div className="divider"/>
