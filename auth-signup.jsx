...
const SignupScreen = ({ onComplete, onSwitchToLogin }) => {
  const { addCustomer, toast } = useStore();
  ...
  const validators = [null, validateStep1, validateStep2, validateStep3, validateStep4, () => f.agreedToTerms ? null : 'You must accept the terms to continue'];
  ...
  const submit = () => {
    ...
    // Register login credential so they can sign in next
    registerUser({
      email: f.email,
      password: f.password,
      role: 'Customer',
...
// ── Step 2 (Individual) ─────────────────────────────────────────────────
const SignupStepIndividual = ({ f, set }) => (
  <div className="flex col gap-3">
...
// ── Step 2 (Organization) ───────────────────────────────────────────────
const SignupStepOrganization = ({ f, set }) => (
  <div className="flex col gap-3">
...
// ── Step 4 (KYC) ────────────────────────────────────────────────────────
const SignupStepKYC = ({ f, set }) => {
  const uploadField = (key, label, hint) => (
...
Object.assign(window, { SignupScreen, SignupSuccess });
