// VM records list + detail drawer — with full CRUD actions

const VMList = ({ openVM, openModal }) => {
  const { state, bulkAction, setVMStatus, deleteVM } = useStore();
  ...
  const filters = [
    ...
    { id: 'expiring', label: 'Expiring ≤ 7d', count: state.vms.filter(v => v.expiry !== '—' && (new Date(v.expiry) - window.MOCK.TODAY) / 86400000 <= 7 && (new Date(v.expiry) - window.MOCK.TODAY) / 86400000 >= 0).length },
  ];
    ...
    <div className="content">
      <div className="page-head">
        ...
        <div className="page-actions">
          <button className="btn" onClick={() => useStore().toast('VMs CSV download started', 'info')}><Icon name="download" size={13}/>Export CSV</button>
      ...
      <div className="card">
        ...
        {selected.size > 0 && (
          <div style={{ padding: '10px 18px', background: 'var(--accent-soft)', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button className="btn sm" onClick={() => { useStore().toast(`${selected.size} VMs exported to CSV`, 'info'); }}>Export selected</button>
...
const VMDrawer = ({ vmId, onClose, openCust, openModal }) => {
  const { state, setVMStatus, deleteVM } = useStore();
  const v = state.vms.find(x => x.id === vmId);
  if (!v) return null;
  const c = state.customers.find(c => c.id === v.customer);
  const [tab, setTab] = React.useState('overview');

  const vmInvoices = state.invoices.filter(i => i.vms.includes(vmId));
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 22 }}>
          {tab === 'overview' && (
            <div className="flex col gap-4">
              ...
              {v.notes && <div className="card"><div className="card-head"><h3 className="card-title">Internal notes</h3></div><div className="card-body"><textarea rows="3" defaultValue={v.notes} onBlur={e => useStore().updateVM(v.id, { notes: e.target.value })} style={{ width: '100%' }}/></div></div>}
            </div>
          )}
            ...
            <div className="flex col gap-4">
              <div className="card"><div className="card-body">
                <dl className="dl">
                  ...
                  <dt>Interconnect</dt><dd>{v.interconnect.length ? v.interconnect.map(id => <span key={id} className="id-tag" style={{marginRight:4}}>{id}</span>) : <span className="text-mute">none</span>}</dd>
                  <dt>Firewall policy</dt><dd className="mono">{v.firewallPolicy}</dd>
                </dl>
              </div></div>
              <div className="card">
                <div className="card-head"><h3 className="card-title">Firewall rules</h3><button className="btn sm" onClick={() => useStore().toast('Open Firewall rules editor from the sidebar', 'info')}><Icon name="edit" size={11}/>Edit</button></div>
          ...
          {tab === 'activity' && (
            <div className="card">
              <div className="card-body">
                {state.activity.filter(a => a.text.includes(v.name) || a.text.includes(v.id)).map((a, i) => (
                  <div key={i} className="feed-item">
                    <span className={`dot ${a.kind}`}/>
                    <div className="body">{a.text}<div className="meta">{a.actor} · {a.ts}</div></div>
                  </div>
                ))}
                {state.activity.filter(a => a.text.includes(v.name) || a.text.includes(v.id)).length === 0 && <div className="empty"><div className="sub">No activity yet.</div></div>}
              </div>
            </div>
          )}
...
Object.assign(window, { VMList, VMDrawer });
