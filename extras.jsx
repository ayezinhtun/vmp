...
const CommandPalette = ({ onClose, setView, openVM, openCust, openModal }) => {
...
const CalendarView = ({ openVM }) => {
  ...
  const firstDay = base.getDay(); // 0=Sun

  // VMs by expiry date in this month
  const byDate = {};
  state.vms.forEach(v => {
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <div key={day} style={{
                ...
                {vms.slice(0, 3).map(v => {
                  const diffDays = Math.ceil((new Date(v.expiry) - today) / 86400000);
      ...
      <div className="mt-4 card">
        ...
        <div className="card-body flush">
          <table className="tbl">
            ...
            <tbody>
              {monthVMs.sort((a, b) => new Date(a.expiry) - new Date(b.expiry)).map(v => {
...
Object.assign(window, { CommandPalette, ShortcutsModal, CalendarView });
