...
const CustomerAccountManagementView = ({ openCust, openModal }) => {
  const { state, updateCustomer, toast, setKYC } = useStore();
  const [selected, setSelected] = React.useState(new Set());
  const [segment, setSegment] = React.useState('all');
  const [search, setSearch] = React.useState('');
  const [view360, setView360] = React.useState(null);
  const [bulkAction, setBulkAction] = React.useState(null);

  // Feature 5: Saved segments
  const segments = [
    { id: 'all', label: 'All customers', filter: () => true },
    { id: 'high-value', label: 'High value (>5M MMK)', filter: c => c.totalSpend > 5000000 },
    { id: 'at-risk', label: 'At risk (KYC issue)', filter: c => c.kyc === 'Rejected' || c.kyc === 'Pending' },
    { id: 'new-30d', label: 'New (last 30 days)', filter: c => (window.MOCK.TODAY - new Date(c.since)) / 86400000 <= 30 },
    { id: 'inactive', label: 'Inactive', filter: c => c.status === 'Inactive' || state.vms.filter(v => v.customer === c.id && v.status === 'Active').length === 0 },
    { id: 'enterprise', label: 'Enterprise (>3 VMs)', filter: c => state.vms.filter(v => v.customer === c.id && v.status === 'Active').length > 3 },
  ];

  const filtered = state.customers
    .filter(segments.find(s => s.id === segment)?.filter || (() => true))
    .filter(c => !search || [c.name, c.company, c.email, c.id, c.phone].join(' ').toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="content" style={{ animation: 'fadeIn 0.3s ease-out' }}>
      <div className="page-head">
        <div>
          ...
          <p className="page-subtitle">Complete customer management · {state.customers.length} total · {state.customers.filter(c => c.kyc === 'Pending').length} pending KYC · MMK {formatMMK(state.customers.reduce((a, c) => a + c.totalSpend, 0))} total lifetime value</p>
        </div>
      ...
      <div className="grid-4 mb-4">
        {[
          { label: 'Total customers', value: state.customers.length, sub: `+${state.customers.filter(c => (window.MOCK.TODAY - new Date(c.since))/86400000 <= 30).length} new this month`, icon: 'users', accent: 'oklch(0.6 0.13 250)' },
          { label: 'Active', value: state.customers.filter(c => c.status === 'Active').length, sub: `${state.customers.filter(c => c.status === 'Inactive').length} inactive`, icon: 'check', accent: 'var(--ok)' },
          { label: 'Pending KYC', value: state.customers.filter(c => c.kyc === 'Pending').length, sub: 'avg 4.2h response', icon: 'shield', accent: 'oklch(0.55 0.16 75)' },
          { label: 'Lifetime value', value: `${formatMMK(Math.round(state.customers.reduce((a,c) => a + c.totalSpend, 0) / 1000000))}M`, sub: 'MMK total', icon: 'invoice', accent: 'oklch(0.55 0.18 285)' },
        ].map((m, i) => (
      ...
      {selected.size > 0 && (
        <div className="card mb-3" style={{ borderColor: 'var(--accent)', background: 'var(--accent-soft)', animation: 'slideIn 0.2s ease-out' }}>
          <div className="card-body" style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button className="btn sm" onClick={() => { toast(`KYC reminder sent to ${[...selected].filter(id => state.customers.find(c => c.id === id)?.kyc === 'Pending').length} customers`, 'ok'); setSelected(new Set()); }}><Icon name="shield" size={11}/>KYC reminder</button>
...
const Customer360 = ({ customer, onClose, openCust }) => {
  const { state, updateCustomer, setKYC, toast } = useStore();
  const c = state.customers.find(x => x.id === customer.id) || customer;
  const vms = state.vms.filter(v => v.customer === c.id);
  const invs = state.invoices.filter(i => i.customer === c.id);
  const tickets = state.tickets.filter(t => t.customer === c.id);
  const tasks = state.tasks.filter(t => t.customer === c.id);
  ...
  const health = Math.round(
    ...
    Math.min(20, vms.filter(v => v.status === 'Active').length * 5) +
    (overdueInv === 0 ? 15 : 0) +
      ...
      <div className="drawer" onClick={e => e.stopPropagation()} style={{ width: 'min(900px, 95vw)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 22 }}>
          ...
          <div className="grid-4 mb-4">
            ...
            <div className="metric"><div className="label">Active VMs</div><div className="value tnum">{vms.filter(v => v.status === 'Active').length}</div><div className="trend">{vms.length} total</div></div>
...
window.CustomerAccountManagementView = CustomerAccountManagementView;
