// Finance: invoices, payments, reports — store-wired

const FinanceView = ({ openCust, openModal }) => {
              ...
              <dl className="dl">
                ...
                <dt>Transaction screenshot</dt><dd>
                  {live.status !== 'Pending' && live.status !== 'Overdue'
                    ...
                    : <button className="btn sm" onClick={() => { updateInvoice(live.id, { status: 'Customer Transferred' }); useStore().toast('Transaction screenshot uploaded', 'ok'); }}><Icon name="attach" size={11}/>Upload</button>}
                </dd>
              </dl>
            </div>
          </div>
        ...
        <div className="card">
          ...
          <div className="card-body flush">
            <table className="tbl">
              ...
              <tbody>
                {state.vms.filter(v => v.expiry !== '—' && (new Date(v.expiry) - window.MOCK.TODAY) / 86400000 <= 30 && (new Date(v.expiry) - window.MOCK.TODAY) / 86400000 >= 0)
                  .sort((a, b) => new Date(a.expiry) - new Date(b.expiry)).map(v => {
            ...
            {[['Dec', 92], ['Jan', 88], ['Feb', 94], ['Mar', 86], ['Apr', 91], ['May', 78]].map(([m, pct]) => (
              <div key={m} className="flex col gap-1" style={{ flex: 1, alignItems: 'center' }}>
                <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'flex-end' }}>
                  <div style={{ width: '100%', height: `${pct}%`, background: pct < 85 ? 'var(--warn)' : 'var(--ok)', borderRadius: '3px 3px 0 0', position: 'relative' }}>
                    <div style={{ position: 'absolute', top: -18, left: 0, right: 0, textAlign: 'center', fontSize: 11, fontWeight: 600 }}>{pct}%</div>
                  </div>
                </div>
...
Object.assign(window, { FinanceView, ReportsView });
