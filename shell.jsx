...
const Sidebar = ({ view, setView, role, roleNames = {}, onAccountClick }) => {
  const auth = typeof useAuth === 'function' ? useAuth() : null;
  const roleLabel = roleNames[role] || role;
  // Temporarily hidden nav items (engineering + infra modules)
  ...
  ] : NAV).filter(it => it.section || !HIDDEN.has(it.id));

  // Role-based gating for admin nav
  const allowedFor = {
  ...
  return (
    <aside className="sidebar">
      ...
      <nav className="nav">
        {items.map((it, i) => {
          if (it.section) {
            // suppress section header if no visible items follow before the next section
            ...
            const visibleSlice = slice.filter(x => !allow || allow.has(x.id));
            if (visibleSlice.length === 0) return null;
            return <div key={i} className="nav-section">{it.section}</div>;
          }
          if (allow && !allow.has(it.id)) return null;
          return (
            <button key={it.id}
      ...
      <div className="nav-user" style={{ cursor: onAccountClick ? 'pointer' : 'default' }} onClick={onAccountClick}>
        ...
        <button className="icon-btn" title="Sign out" onClick={(e) => { e.stopPropagation(); auth?.signout(); }}><Icon name="logout" size={14}/></button>
      </div>
    </aside>
  );
};

const Topbar = ({ crumbs, actions, theme, setTheme, onBellClick, onSearchClick, onHelpClick, unread }) => (
