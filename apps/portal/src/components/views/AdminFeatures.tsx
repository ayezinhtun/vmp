            ...
            {Object.entries(ROLES).map(([k, r]) => (
              <div key={k} style={{ padding: '12px 18px', borderBottom: '1px solid var(--line)' }}>
                <div className="flex center between mb-2">
                  ...
                  <span className="text-xs text-mute tnum">{state.team.filter((u: any) => u.role === k).length} {state.team.filter((u: any) => u.role === k).length === 1 ? 'member' : 'members'}</span>
                </div>
...
const SecurityCol: React.FC<{ title: string; items: [string, boolean][] }> = ({ title, items }) => {
