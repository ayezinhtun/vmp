...
import { Avatar, StatusPill } from '../components/ui/ui'

export const KYCReviewView: React.FC = () => {
  const { customers, setKYC } = useCustomerStore()
  const { toast } = useUIStore()
  const [tab, setTab] = useState('Pending')
  const [selected, setSelected] = useState<any>(null)
  const [note, setNote] = useState('')

  const pending = customers.filter((c: any) => c.kyc === 'Pending')
  const approved = customers.filter((c: any) => c.kyc === 'Approved')
  const rejected = customers.filter((c: any) => c.kyc === 'Rejected')
